...
* `REDIS_TLS_CA` - PEM-encoded certificate for your Certificate Authority, if not included in the default trust store. Must be used in conjunction with `REDIS_USE_TLS` set to `true`.  (disabled by default)
...
* `SENTINEL_TLS_CA` - PEM-encoded certificate for your Certificate Authority, if not included in the default trust store. Must be used in conjunction with `SENTINEL_TLS_ENABLED` set to `true`.  (disabled by default)
