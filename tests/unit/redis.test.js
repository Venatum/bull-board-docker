...
describe("Redis Client", () => {
	...
	const importRedis = async () => {
		importCounter++;
		return await import(`../../src/redis.js?v=${importCounter}`);
	};
	...
	beforeEach(() => {
		mock.restore();
		mockClient = { on: mock() };
		createClientMock = mock().mockReturnValue(mockClient);
	});
	...
	describe("Basic Configuration", () => {
		it("should create a Redis client with the correct configuration", async () => {
			setupCommonMocks();
			const { redisConfig, client } = await importRedis();
		...
		it("should use mock client in test environment", async () => {
			setupCommonMocks();
			const { client } = await importRedis();
	...
	describe("Sentinel Configuration", () => {
		it("should parse DSN to sentinels correctly", async () => {
			...
			const { redisConfig } = await importRedis();
	...
	describe("Cluster Configuration", () => {
		it("should parse cluster hosts correctly", async () => {
			...
			const { clusterConfig, isCluster } = await importRedis();
		...
		it("should handle semicolon-separated cluster hosts", async () => {
			...
			const { clusterConfig } = await importRedis();
		...
		it("should include cluster options with correct defaults", async () => {
			...
			expect(clusterConfig.options.redisOptions).toEqual(
				expect.objectContaining({ password: "secret", maxRetriesPerRequest: null }),
			);
		});
		...
		it("should throw on invalid NAT map JSON", async () => {
			...
			await expect(importRedis()).rejects.toThrow("Invalid REDIS_CLUSTER_NAT_MAP JSON");
		});
		...
		it("should include TLS options in cluster redisOptions", async () => {
			...
			expect(clusterConfig.options.redisOptions.tls).toEqual(
				expect.objectContaining({ ca: "ca-cert", servername: "redis.cluster.local" }),
			);
		});
		...
		it("should set isCluster to false when REDIS_CLUSTER_HOSTS is not set", async () => {
			...
			const { isCluster, clusterConfig } = await importRedis();
	...
	describe("TLS Configuration", () => {
		...
		it("should not include TLS options when REDIS_USE_TLS is false", async () => {
			setupCommonMocks({ REDIS_USE_TLS: false });
			const { redisConfig } = await importRedis();
