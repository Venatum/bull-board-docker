import { existsSync as _realExistsSync, readFileSync as _realReadFileSync } from "node:fs";
import { describe, expect, it, beforeEach, afterAll, mock } from "bun:test";

// Capture real fs functions before any mocking to avoid circular references
const realExistsSync = _realExistsSync;
const realReadFileSync = _realReadFileSync;

const isTlsTestPath = (path) =>
	typeof path === "string" &&
...
describe("Configuration", () => {
	...
	const setupFsMock = () => {
		mock.module("node:fs", () => ({
			existsSync: (path) => (isTlsTestPath(path) ? true : realExistsSync(path)),
			readFileSync: (path, encoding) =>
				isTlsTestPath(path) ? path : realReadFileSync(path, encoding),
		}));
	};
	...
	const getConfig = async () => {
		importCounter++;
		const { config, PROXY_PATH } = await import(`../../src/config.js?v=${importCounter}`);
