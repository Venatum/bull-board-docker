...
| [`bump.yml`](./workflows/bump.yml)                     | Manual                                                      | Bump dependencies to their latest minor (ncu), run lint/format/test + Docker build + Playwright e2e smoke test, then commit and push a `chore(deps)` commit directly to `master` (triggering `release.yml`). |
